          ...
          : <fieldset disabled={saving || uncertain} className="min-w-0 space-y-5">
            ...
            {step === 1 && <>
              ...
              {!day || day < today ? <p role="status">Elige una fecha a partir de hoy.</p>
                ...
                : !slots.length ? <div role="status" className="rounded-xl bg-subtle p-4 space-y-2"><p className="font-semibold">No hay horarios disponibles para esta fecha.</p><p className="text-body text-muted">Prueba otro día o vuelve atrás y elige cualquier profesional. Si no encuentras disponibilidad, contacta al negocio para coordinar tu cita.</p><Button variant="ghost" onClick={() => setSlotsRetry(v => v + 1)}>Actualizar horarios</Button></div>
